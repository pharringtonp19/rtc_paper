...
\begin{itemize}
    \item \cite{evans2019reducing}: rapid re-housing programs, which provide immediate
temporary subsidies for market housing, have replaced many traditional programs for people experiencing
somewhat less chronic situations of homelessness. 
\end{itemize}
...
\begin{align*}
\intertext{Choice Set}
   &x := (\textrm{quality}, \textrm{price}, \textrm{search period}, \dots ) \in \mathcal{X}\\ 
\intertext{Constraint Function}
&F :: \textrm{Params} \to  \textrm{RTC} \to \mathcal{X} \to \{0, 1\} \\ 
\intertext{Utility Function}
&U :: \textrm{Params} \to \mathcal{X} \to \mathcal{R}_+ \\ 
\intertext{Pre-Policy Problem}
&\underset{x \in F_{p, 0}^{-1}(0)}{\textrm{maximize}} \ U_{\alpha} (x) \\ 
\intertext{Implicit Function of Interest}
& s^*(\textrm{rtc}, p, \alpha) := \underset{x \in F_{p, \textrm{rtc}}^{-1}(0)}{\textrm{argmax}_2} \ U_{\alpha} (x) \\ 
\intertext{Estimand}
...
\section{Conclusion}
\begin{itemize}
    \item \cite{evans2019reducing}: ``Direct federal
funding for local organizations to combat homelessness totals about \$6.1 billion annually (USICH 2018)''
\end{itemize}
