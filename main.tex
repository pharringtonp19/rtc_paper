...
In this paper, where we're interested in studying the effects of the Right to Counsel at scale, we take as our intermediate measurement the length of the housing search process for low-income individuals. As explained in the paper, this is a far from perfect or comprehensive outcomes variable. We regard it a suggestive signal in the sense that if there are adverse effects of the policy -- if landlords decide to re-optimize in response to the Right to Counsel -- we would see it via the housing search channel. \par 

And indeed, while extremely preliminary, we see some indication that there may be adverse effects along this channel. We caution, though, that our results are preliminary.  
