...
Deep learning models are often touted for their ability to drasticially improve their performance with the size of the data. In this paper, we leverage them for an alternative reason. We use them because they allow us to encode meaningful priors in a controllable fashion. This quasi-Bayesian viewpoint of learning/estimation in the finite sample is in line with modern deep learning works (\cite{belkin2018reconciling}, \cite{wilson2020case}, \cite{nagarajan2019uniform}) which emphasize the importance of ``understanding the nature of the inductive bias''\footnote{\cite{belkin2021fit} ``the properties that make some solutions preferable to others despite all of them fitting the training data equally''} of the estimator and differentiates this work from recent theoretical econometric work that seeks to reduce the influence of regularization on the estimation process (\cite{farrell2021deep}, \cite{chernozhukov2018double}).  Our high level aim is that we want an empirical estimation framework that can seamlessly account for clustered data (which is ubiquitous in applied work) in a flexible way; that has a clear inductive bias; and is conceptually simple to understand in a way so that it can be easily extended to account for various other issues that an applied microeconomist might encounter. Targeted (handle cluster data), interpretable (clear inductive bias)  
and extendable (self-explanatory).\par 
